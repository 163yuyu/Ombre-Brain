...
- GitHub 同步会把事件 Markdown 与 `_sources` 原文明文提交到已配置仓库。它们不是端到端加密备份；生产上应使用可信私有仓库并审计协作者权限，绝不能把包含私密对话的 path prefix 放进公开仓库。
- 配置 `OMBRE_GITHUB_STATE_KEY` 后，OAuth 动态客户端与 grant 状态会作为独立 AES-GCM 密文备份；代码会拒绝把这份状态上传到公开仓库。密钥必须稳定保存于部署环境，不能提交到 Git。
...
8. 启用 `OMBRE_GITHUB_AUTO_RESTORE` 时，生命周期先恢复空的记忆目录和缺失的 OAuth 状态，再启动 MCP 与定时同步；恢复失败会让启动失败关闭，而不是用空本地目录生成新的权威清单。
